    ...
    if request.method == "POST":
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash(f"Congratulations! You purchased "
                      f"{p_item_object.name} for ${p_item_object.price}",
                      category='success')
            else:
                flash(f"Unfortunately, you do not have enough money to purchase "
                      f"{p_item_object.name}!", category='danger')
        # Sold item logic
        ...
        if s_item_obj:
            if current_user.can_sell(s_item_obj):
                ...
                flash(f"Congratulations! You have successfully sold "
                      f"{s_item_obj.name} back to the market for ${s_item_obj.price}",
                      category='success')
            else:
